...
const JobPortalPage = () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const uniqueIndustries = [
            ...new Set(jobsList.map((job) => job.industry).filter(Boolean)),
          ];
  ...
  const applyFilters = useCallback(() => {
    const results = jobs.filter((job) => {
      ...
      const salaryMatch =
        !job.salary ||
        (job.salary.min || 0) >= salaryRange.min &&
          (job.salary.max || 500000) <= salaryRange.max;

      return (
        titleMatch && locationMatch && industryMatch && jobTypeMatch && salaryMatch
      );
    });
